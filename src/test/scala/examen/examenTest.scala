...
class examenTest extends TestInit {
  ...
  println("Elaborado por: Alfredo Naranjo Serres")
...
//
  ...
  "Ejercicio 5" should "calcula el ingreso total" in {
    ...
    val ventas = spark.read.options(
      Map(
        "header" -> "true",
        "inferSchema" -> "true"
      )
    ).csv("src/test/resources/ventas.csv").toDF()

    val out = ejercicio5(ventas).collect().map(x => (x.get(0), x.get(1)))
